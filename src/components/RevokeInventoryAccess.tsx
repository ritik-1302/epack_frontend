      ...
      setLoading(true)
        try{
            const response = await fetch(
              `${baseURL}/revoke_inventory_access`,
              {
            ...
            if (response.status===200){
              alert("Sucessfully revoked Access to the Inventory")
    ...
    <Dialog>
          ...
          <div className="w-[436px] cursor-pointer p-[20px] border border-gray-300 rounded-md shadow-sm flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <h1 className="text-[14px] font-normal text-gray-600">
                Revoke Inventory Access to a user
              </h1>
            </div>
          </div>
        </DialogTrigger>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <DialogFooter>
            ...
            >
            {loading ? <CircularProgress size={20} color="inherit" /> : "Revoke Inventory Access"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
